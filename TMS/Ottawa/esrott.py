...
PORT = 8876
map=sumolib.net.readNet("ottawa.net.xml")
...
def sendEmergencyVehicleFixed (vehicles) :
	...
	track_vehicles(traci.vehicle.getRoute("0a"), "0a")
	if erp >= 3 :
...
tracked_vehicles= []	
#track vehicles
def track_vehicles(route, ambulanceID) :	
	global tracked_vehicles 
	for edge in route:
		vehicles = traci.edge.getLastStepVehicleIDs(edge)
		for v in vehicles:
			if v is not ambulanceID:
				tracked_vehicles.append((v,20))
...
def getNextLight(edge1, edge2) :
		if edge1 :
			if map.getEdge(edge1).getTLS() != None:
				tl=str(sumolib.net.TLS.getID(map.getEdge(edge1).getTLS()))
				print tl
...
def run(ambulanceID, erp) :
	...
	while traci.simulation.getMinExpectedNumber() > 0:
		...
		for v in tracked_vehicles :
			if v[0] in traci.simulation.getArrivedIDList() :
				print "vehicle : ",v, " left after ", step-20
		if ambulanceID in vehicles :
			if(traci.vehicle.getRoadID(ambulanceID) != ambulanceEdge) & (traci.vehicle.getRoadID(ambulanceID) in traci.vehicle.getRoute(ambulanceID)): #detect if ambulance has reached new edge
				...
				revertLight(prevEdge,getNextEdge(ambulanceID) ,phaseDuration)
...
if __name__ == "__main__":
	...
	sumoProcess = subprocess.Popen([sumoBinary, "-c", "ottawa.sumocfg", "--tripinfo-output", "tripinfo.xml", "--remote-port", str(PORT)], stdout=sys.stdout, stderr=sys.stderr)
	run("",-1)
	#sumoProcess.wait()
