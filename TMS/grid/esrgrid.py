...
def sendEmergencyVehicleFixed (vehicles) :
	...
	erp=chooseERP(traci.vehicle.getRoute("0a"), 2)
	if erp >= 3 :
...
if __name__ == "__main__":
	...
	sumoProcess = subprocess.Popen([sumoBinary, "-c", "gridcfg.sumocfg", "--tripinfo-output", "tripinfo.xml", "--remote-port", str(PORT)], stdout=sys.stdout, stderr=sys.stderr)
	run("",-1)
	#sumoProcess.wait()
